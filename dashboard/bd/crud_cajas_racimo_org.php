...
try {
    switch($opcion){
        case 1: // Alta
            if (!empty($tipo_caja)) {
                $consulta = "INSERT INTO bd_empaque.caja_racimo_org (tipo_caja) VALUES(:tipo_caja)";
        ...
        case 2: // Modificación
                if (!empty($tipo_caja)) {
                    ...
                    $consulta = "UPDATE bd_empaque.caja_racimo_org SET tipo_caja=:nuevo_tipo_caja WHERE tipo_caja=:tipo_caja"; // Modificamos la consulta de actualización
                    $resultado = $conexion->prepare($consulta);
                    if ($resultado->execute(array(':nuevo_tipo_caja' => $nuevo_tipo_caja, ':tipo_caja' => $tipo_caja))) {
        ...
        case 3: // Baja
            if (!empty($tipo_caja)) {
                $consulta = "DELETE FROM bd_empaque.caja_racimo_org WHERE tipo_caja=:tipo_caja";
                $resultado = $conexion->prepare($consulta);
                $resultado->execute(array(':tipo_caja' => $tipo_caja));
